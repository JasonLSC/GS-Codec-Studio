...
from gsplat.compression.yuv_utils import _save_tensor_to_yuv, _encode_yuv_to_video_stream, \
    _decode_video_stream_to_yuv, _load_yuv_to_tensor, gaussian_downsample, chroma_downsampling

@dataclass
class SeqYUVCompression:
    ...
    Args:
        ...
        color_standard (str, optional): The color standard to use for RGB-YCbCr conversion. 
                                       Options: "BT709" (default) or "BT470".
    """
    ...
    def __post_init__(self, attribute_codec_registry):
        if attribute_codec_registry:
            available_functions = {
                ...
                "_compress_video_yuv": _compress_video_yuv,
                "_compress_shN_video_yuv": _compress_shN_video_yuv,
                ...
                "_decompress_video_yuv": _decompress_video_yuv,
                "_decompress_shN_video_yuv": _decompress_shN_video_yuv,
            }
    ...
    def compress(self, compress_dir: str) -> None:
        ...
        for param_name in self.splats_videos.keys():
            ...
            if param_name in ["sh0", "shN"]:
                if self.chroma_subsampling[param_name] == "420":
                    kwargs["chroma_subsampling"] = "420"
                if self.use_chroma_qp_offset[param_name]:
                    kwargs["use_chroma_qp_offset"] = True
...
def _compress_video_yuv(
        ...
        debug: bool = False,
        use_all_intra: bool = False,
        chroma_subsampling: Literal["420", "444"] = "444",
    ...
    if chroma_subsampling == "420":
        grid_norm = chroma_downsampling(grid_norm)
    video = (grid_norm * (2**8 - 1)).round().to(torch.uint8)

    # uncomment to debug
    # torch.save(video, os.path.join(compress_dir, f"{param_name}.pt"))
    
    # Save tensor to YUV file
    intermediate_data_dir = os.path.join(os.path.dirname(compress_dir), "intermediate_data")
    os.makedirs(intermediate_data_dir, exist_ok=True)
    yuv_file = os.path.join(intermediate_data_dir, f"{param_name}.yuv")
    _save_tensor_to_yuv(video, yuv_file, chroma_subsampling)
    
    # Video encoding
    pix_fmt = f"yuv{chroma_subsampling}p"
    file_extension = ".265" if debug else ".mp4" # Choose output extension based on debug flag
    video_file = os.path.join(compress_dir, f"{param_name}{file_extension}")
    height, width = video.shape[1:3]
    try:
        # Call the dedicated encoding function
        _encode_yuv_to_video_stream(
            ...
            pix_fmt=pix_fmt,
            qp=qp,
            use_all_intra=use_all_intra,
            use_chroma_qp_offset=use_chroma_qp_offset
        )
    finally:
        # Always attempt cleanup unless in debug mode
        if not debug and os.path.exists(yuv_file):
            try:
                os.remove(yuv_file)
                print(f"Removed intermediate file: {yuv_file}")
            except OSError as e:
                # Log error but don't prevent returning metadata
                print(f"Warning: Error removing intermediate file {yuv_file}: {e}")
...
def _decompress_video_yuv(
    ...
    os.makedirs(intermediate_data_dir, exist_ok=True)
    # Video decoding
    video_file = os.path.join(compress_dir, f"{param_name}.mp4")    
    yuv_file = os.path.join(intermediate_data_dir, f"{param_name}_decoded.yuv")
    height, width = meta["shape"][1:3]
    _decode_video_stream_to_yuv(
    ...
    # Load yuv file into tensor
    video = _load_yuv_to_tensor(yuv_file, height, width, meta["pix_fmt"])
    # os.remove(yuv_file)

    # uncomment to debug
    # video = torch.load(os.path.join(compress_dir, f"{param_name}.pt")).to("cpu")
...
def _compress_shN_video_yuv(
    ...
    # Chroma subsampling
    if chroma_subsampling == "420":
        grid_norm = chroma_downsampling(grid_norm) # takes [T, H, W, 15, 3] as input 
    shN_video = (grid_norm * (2**8 - 1)).round().to(torch.uint8)

    # Save SHN into YUV files and encode them
    intermediate_data_dir = os.path.join(os.path.dirname(compress_dir), "intermediate_data")
    os.makedirs(intermediate_data_dir, exist_ok=True)
    file_extension = ".265" if debug else ".mp4" # Choose output extension based on debug flag
    pix_fmt = f"yuv{chroma_subsampling}p"
    for shN_id, shN_name in enumerate(shN_name_list):
        ...
        shN_video_yuv = shN_video[..., shN_id, :]
        # Save SHN into YUV files
        yuv_file = os.path.join(intermediate_data_dir, f"{param_name}_{shN_name}.yuv")
        _save_tensor_to_yuv(shN_video_yuv, yuv_file, chroma_subsampling)
        # Video encoding
        video_file = os.path.join(compress_dir, f"{param_name}_{shN_name}{file_extension}")
        ## Call the dedicated encoding function
        _encode_yuv_to_video_stream(
            yuv_file_path=yuv_file,
            output_video_path=video_file,
            ...
            height=height,
            pix_fmt=pix_fmt,
            qp=qp[shN_name[0:3]], # qp at this degree
            use_all_intra=use_all_intra,
            use_chroma_qp_offset=use_chroma_qp_offset
        )
...
def _decompress_shN_video_yuv(
    ...
    for shN_id, shN_name in enumerate(shN_name_list):
        video_file = os.path.join(compress_dir, f"{param_name}_{shN_name}{file_extension}")
        yuv_file = os.path.join(intermediate_data_dir, f"{param_name}_{shN_name}_decoded.yuv")
        ...
        shN_video = _load_yuv_to_tensor(yuv_file, width, height, pix_fmt)
        shN_video_list.append(shN_video)
    shN_videos = torch.stack(shN_video_list, dim=3)

    grid_norm = shN_videos / (2**8 - 1)
    mins = torch.tensor(meta["mins"])
    maxs = torch.tensor(meta["maxs"])
...
def _decompress_video_hevc_16bit(
    ...
    # cal_psnr = lambda x, y: float('inf') if (d := np.mean((x-y)**2)) == 0 else 20*np.log10(d)
