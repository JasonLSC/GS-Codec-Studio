...
@dataclass
class FreeTimeGS_Strategy(Strategy):
    '''A modified densification and pruning strategy of FreeTimeGS paper:
    ...
    def check_sanity(
        ...
        Check if:
            ...
            * The following keys are present: {"means", "scales", "quats", "opacities", "t_center", "t_scale", "motion"}.
        ...
        for key in ["means", "scales", "quats", "opacities", "t_center", "t_scale", "motion",]:
    ...
    def step_pre_backward(
        ...
        if self.temp_vis_mask:
            assert (
                "t_vis_mask" in info
            ), "The temporal visible mask of the Gaussians is required but missing."

            old_tensor = info["means2d"]

            # re-assign the "info["means2d"]" to make 
            n_gaussian = len(list(params.values())[0])

            info["means2d"] = torch.zeros([old_tensor.size()[0],n_gaussian,2], device=old_tensor.device)
