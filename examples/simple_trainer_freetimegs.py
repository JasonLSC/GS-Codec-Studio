...
from gsplat.strategy import FreeTimeGS_Strategy # import densification and pruning strategy that fits STG model
...
class Config:
    ...
    batch_size: int = 2 
    feature_dim: Optional[int] = None # Note: None means that we use default SH
    device: str = "cuda"
    ...
    # )
    strategy: Literal["FreeTimeGS_Strategy"] = "FreeTimeGS_Strategy"
    ...
    opa_4d_reg_lambda: float = 0.01

    # perceptual loss
    perceptual_loss_lambda: float = 0.01

    
    
# Initialize splats with optimizers
def create_splats_with_optimizers(
    ...
    t_center = times.contiguous() # [N, 1]

    # t_scale is related to life-span
    t_scale = torch.log(torch.ones((N, 1))) # [N, 1]

    # Linear motion, so shape in (N,3)
    motion = torch.zeros((N, 3))

    # Do not allow orientation change, so comment
    ...
    params = [
        ...
        ("t_scale", torch.nn.Parameter(t_scale), cfg.trbfs_lr),
        ("t_center", torch.nn.Parameter(t_center), cfg.trbfc_lr),
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.splats, self.optimizers = create_splats_with_optimizers(
            ...
            feature_dim=self.cfg.feature_dim, # it should be "None" in FreeTimeGS impl.
        ...
        if cfg.strategy == "FreeTimeGS_Strategy":
            self.strategy = FreeTimeGS_Strategy(
                verbose=True, 
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if not self.cfg.compression_sim:
            ...
            t_center = self.splats["t_center"] # [N, 1]
            t_scale = torch.exp(self.splats["t_scale"]) # [N, 1]
            motion = self.splats["motion"] # [N, 3]
            ...
            t_center = self.comp_sim_splats["t_center"] # [N, 1]
            t_scale = torch.exp(self.comp_sim_splats["t_scale"]) # [N, 1], log domain
            motion = self.comp_sim_splats["motion"] # [N, 3]

            sh0, shN = self.comp_sim_splats["sh0"], self.comp_sim_splats["shN"]

        # temporal info.
        t_offset = timestamp - t_center
        t_opa_decay = torch.exp(-0.5 * (t_offset / t_scale) ** 2) # temporal opacity decay

        # opacity at this timestamp = point opacity * temporal opacity decay
        opacity = opacities * t_opa_decay.squeeze()

        # position at this timestamp = point position + motion * t_offset
        means_motion = means + motion[:, 0:3] * t_offset
        ...
        render_colors, render_alphas, info = rasterization(
            means=means_motion, # input moved position
            quats=quats, 
            scales=scales,
            opacities=opacity, # input temporal opacity
            colors=colors, # input SH coeffs
        ...
        loss_data = {
            "opacities": opacities,
            "t_opacities": t_opa_decay,            
        }

        # pixels(GT) shape: [1, H, W, 3]
        return render_colors, render_alphas, info, loss_data
    ...
    def train(self):
        ...
        with self.get_profiler(self.writer) as prof:
            ...
            for batch in self.trainloader:
                ...
                # forward
                renders, alphas, info, loss_data = self.rasterize_splats(
                ...
                # perceptual loss
                perceptual_loss = self.lpips(colors.permute(0, 3, 1, 2), pixels.permute(0, 3, 1, 2))
                loss = loss + cfg.perceptual_loss_lambda * perceptual_loss

                # 4d regularization loss
                opa_4d_reg_loss = (loss_data["opacities"] * loss_data["t_opacities"].detach()).mean()
                loss = loss + cfg.opa_4d_reg_lambda * opa_4d_reg_loss
                ...
                if isinstance(self.strategy, FreeTimeGS_Strategy):
    ...
    @torch.no_grad()
    def export_dyn_splats_to_ply_sequence(self,):
        ply_dir = f"{cfg.result_dir}/plys"
    ...
    @torch.no_grad()
    def get_sliced_splats_from_dyn_splats(self, splats, timestamp, opa_vis_mask = True):
        '''
        ...
        trbfcenter = splats["trbf_center"] # [N, 1]
        trbfscale = torch.exp(splats["trbf_scale"]) # [N, 1]
        
        motion = splats["motion"] # [N, 9]
        omega = splats["omega"] # [N, 4]
        color = splats["rgb"] # [N, 3] 
    ...
    def save_static_splats_to_ply(self, path, splats, mode="splats", use_text=False):
        ...
        opacities = splats["opacities"].unsqueeze(1).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    configs = {
        "default": (
            "FreeTimeGS training from the original paper.",
            Config(
            ),
        ),
        "compression_sim": (
            "FreeTimeGS training and compression simulation",
            Config(
