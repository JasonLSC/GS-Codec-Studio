...
# Interactive pipeline script to run main-track video experiments and generate metrics
...
LOGS_DIR="logs/main_track_pipeline_interactive_$(date '+%Y%m%d_%H%M%S')"
mkdir -p "$LOGS_DIR"

print_status "🚀 Interactive Main Track Video Pipeline Execution"
print_status "Logs will be saved to: $LOGS_DIR"
echo

# Step selection
echo -e "${BLUE}Available steps:${NC}"
echo "  1. Run main-track video experiments (run_main_track_exps.sh)"
...
# Step 1
if ask_confirmation "Execute Step 1: Run main-track video experiments?"; then
    print_status "Step 1/4: Running main-track video experiments..."
    if bash scripts/main_track_vid/run_main_track_exps.sh 2>&1 | tee "$LOGS_DIR/step1_run_main_track_exps.log"; then
        print_success "Step 1 completed: Main-track video experiments finished"
    else
        print_error "Step 1 failed: Main-track video experiments failed."
    ...
    print_warning "Skipping Step 1: Main-track video experiments"
    echo
fi
...
if ask_confirmation "Execute Step 2: Add camera info to decoded PLY files?"; then
    ...
    if bash scripts/main_track_vid/add_cam_info_to_dec_ply.sh 2>&1 | tee "$LOGS_DIR/step2_add_cam_info.log"; then
...
if ask_confirmation "Execute Step 3: Run MPEG-GSC metrics?"; then
    ...
    if bash scripts/main_track_vid/run_mpeg_gsc_metrics.sh 2>&1 | tee "$LOGS_DIR/step3_mpeg_gsc_metrics.log"; then
...
if ask_confirmation "Execute Step 4: Extract metrics and merge to Excel?"; then
    ...
    if bash scripts/main_track_vid/run_metrics_extraction.sh 2>&1 | tee "$LOGS_DIR/step4_metrics_extraction.log"; then
...
# Final summary
print_success "🎉 Interactive main track video pipeline execution completed!"
