...
if bash scripts/1f_vid/run_1f_exps.sh > "$LOGS_DIR/step1_run_1f_exps.log" 2>&1; then
...
if bash scripts/1f_vid/add_cam_info_to_dec_ply.sh > "$LOGS_DIR/step2_add_cam_info.log" 2>&1; then
...
if bash scripts/1f_vid/run_mpeg_gsc_metrics.sh > "$LOGS_DIR/step3_mpeg_gsc_metrics.log" 2>&1; then
...
if bash scripts/1f_vid/run_metrics_extraction.sh > "$LOGS_DIR/step4_metrics_extraction.log" 2>&1; then
