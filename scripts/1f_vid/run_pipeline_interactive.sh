...
if ask_confirmation "Execute Step 1: Run 1-frame experiments?"; then
    ...
    if bash scripts/1f_vid/run_1f_exps.sh 2>&1 | tee "$LOGS_DIR/step1_run_1f_exps.log"; then
...
if ask_confirmation "Execute Step 2: Add camera info to decoded PLY files?"; then
    ...
    if bash scripts/1f_vid/add_cam_info_to_dec_ply.sh 2>&1 | tee "$LOGS_DIR/step2_add_cam_info.log"; then
...
if ask_confirmation "Execute Step 3: Run MPEG-GSC metrics?"; then
    ...
    if bash scripts/1f_vid/run_mpeg_gsc_metrics.sh 2>&1 | tee "$LOGS_DIR/step3_mpeg_gsc_metrics.log"; then
...
if ask_confirmation "Execute Step 4: Extract metrics and merge to Excel?"; then
    ...
    if bash scripts/1f_vid/run_metrics_extraction.sh 2>&1 | tee "$LOGS_DIR/step4_metrics_extraction.log"; then
